...
class Preprocessor:
    ...
    def preprocess(self):
        ...
        for root, dirs, files in os.walk(self.img_dir):
            ...
            # print(files)
            pbar = tqdm(total=other_data_size if curr_dir == 'other_faces' else data_size)
            for file in files:
                pbar.update()
                if file.endswith(self.ext):  # walk through all the images.ext in img_dir
                    # print(file)
                    label = 0 if curr_dir == 'other_faces' else 1
                    path = os.path.join(root, file)
                    raw_image = cv2.imread(path)
                    # image = cv2.resize(raw_image, (300, 300))
                    (h, w) = raw_image.shape[:2]
                    blob = cv2.dnn.blobFromImage(raw_image, 1.0, (300, 300) if not curr_dir == 'other_faces' else
                                                 (270, 300), (104.0, 117.0, 123.0))
                    ...
                    for i in range(0, detections.shape[2]):
                        ...
                        face_resize = cv2.resize(face_gray, self.data_size)

                        data_flattened = face_resize.flatten()
                        x_train.append(data_flattened)
                        y_labels.append(label)
                        if curr_dir == 'other_faces':
                            other_data_count += 1
                        else:
                            data_count += 1
                        if data_count > data_size or other_data_count > other_data_size:
                            pbar.close()
                            break
                    if data_count > data_size or other_data_count > other_data_size:
                        break
        ...
        y_labels_mod = []
        for n, str in enumerate(self.label_ids):
            for labels in y_labels:
                if labels == str:
                    y_labels_mod.append(n)
        y_labels = y_labels_mod
...
class DataSet:
    def __init__(self, inputs, labels, class_num, batch_size=None):
        # assert inputs.shape[0] == len(outputs)
        ...
        self.batch_count = inputs.shape[0] // batch_size
